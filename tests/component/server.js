...
app.post('/test/seed', (req, res) => {
  ...
  const sp = db.prepare('INSERT INTO sportsmen (name, club, category) VALUES (?, ?, ?)').run('Alice', 'Test Club', 'Junior Women');
  const sp2 = db.prepare('INSERT INTO sportsmen (name, club, category) VALUES (?, ?, ?)').run('Bob', 'Test Club 2', 'Junior Men');
...
app.post('/test/seed/scored', (req, res) => {
  ...
  const round = db.prepare('INSERT INTO rounds (competition_id, name, round_order) VALUES (?, ?, ?)').run(comp.lastInsertRowid, 'Finals', 1);

  function addAthlete(name, startOrder, scores) {
    const sp = db.prepare('INSERT INTO sportsmen (name) VALUES (?)').run(name);
    ...
    scores.forEach((score, i) => {
      const attempt = db.prepare('INSERT INTO attempts (entry_id, attempt_number) VALUES (?, ?)').run(entry.lastInsertRowid, i + 1);
  ...
  res.json({ roundId: Number(round.lastInsertRowid) });
});
